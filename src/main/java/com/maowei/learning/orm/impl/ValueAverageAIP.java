...
public class ValueAverageAIP implements FundAIPCalculateInf {
    public FundAIPResult doCalculate(FundAIPResult old, FundInfo fundInfo) {
        ...
        //当期赎回金额
        double currSold = Math.max(0-different, 0);
        double totalSold = old.getTotalSold() + currSold;
        newAIP.setCurrSold(currSold);

        //当期投入金额
        double currInput = Math.max(different, 0);
        ...
        //当期买入份额
        double currShare = different/fundInfo.getNetValue().doubleValue();
